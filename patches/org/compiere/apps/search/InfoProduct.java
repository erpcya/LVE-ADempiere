...
public class InfoProduct extends Info implements ActionListener, ChangeListener
{
	...
	private static final String s_productFrom =
		...
		//Tax Amt Column
		+ " left outer join C_Tax tax on (p.C_TaxCategory_ID = tax.C_TaxCategory_ID) "
		;
	...
	{
		int precision = MCurrency.getStdPrecision(Env.getCtx(), Env.getContextAsInt(Env.getCtx(), "$C_Currency_ID"));
		...
		if (s_productLayout == null)
		{
			...
			//2014-02-11 Carlos Parada Add Column Tax  
			list.add(new Info_Column(Msg.translate(Env.getCtx(), "C_Tax_ID"), "tax.Name", String.class));
			list.add(new Info_Column(Msg.translate(Env.getCtx(), "Rate"), "tax.Rate", BigDecimal.class));
			list.add(new Info_Column(Msg.translate(Env.getCtx(), "TaxAmt"), "Round(bomPriceList(p.M_Product_ID, pr.M_PriceList_Version_ID) * ((tax.Rate) /100)," + precision + ") ", BigDecimal.class));
			list.add(new Info_Column(Msg.translate(Env.getCtx(), "Price"), "Round(bomPriceList(p.M_Product_ID, pr.M_PriceList_Version_ID) + (bomPriceList(p.M_Product_ID, pr.M_PriceList_Version_ID) * ((tax.Rate) /100)) ," + precision + ") ", BigDecimal.class));
			//End Carlos Parada
