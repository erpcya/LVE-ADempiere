...
public class CostCreate extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		//2014-06-30
		//	Carlos Parada 
		// 	Comment allow all products to cost create
		...
		if (product.get_ID() != p_M_Product_ID)
			throw new AdempiereUserError("@NotFound@: @M_Product_ID@ = " + p_M_Product_ID);*/
		int l_Updated = 0;
		List<MProduct> products = new Query(getCtx(), MProduct.Table_Name,"(M_Product_ID=? OR 0=?) AND AD_Client_ID=?", get_TrxName())
		...
		for (MProduct product : products)
		{
			if (!MCostDetail.processProduct(product, get_TrxName()))
				addLog("@Error@ " + product.getValue() + " " + product.getName() );
			else
				l_Updated++;
		}
		return "@Updated@ " + l_Updated;
	}	//	doIt
