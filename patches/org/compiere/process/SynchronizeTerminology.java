...
public class SynchronizeTerminology extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		try {
			...
			sql = "SELECT COUNT(*) FROM AD_EntityType WHERE EntityType = 'ECA01'";
	...
	*/
	...
	/*
				...
				*/
				...
				//	Update translation in reports
				log.info("Synchronize reports Translations");
				sql="UPDATE AD_PRINTFORMATITEM_TRL trl SET PrintName = (SELECT e.PrintName FROM AD_ELEMENT_TRL e "
						+ "INNER JOIN AD_COLUMN c ON(c.AD_Element_ID = e.AD_Element_ID) "
						+ "INNER JOIN AD_PRINTFORMATITEM pfi ON(pfi.AD_Column_ID = c.AD_Column_ID) "
						+ "WHERE e.AD_LANGUAGE=trl.AD_LANGUAGE AND pfi.AD_PrintFormatItem_ID=trl.AD_PrintFormatItem_ID)"
						+ "WHERE EXISTS(SELECT 1 FROM AD_PRINTFORMATITEM pfi "
						+ "WHERE pfi.AD_PRINTFORMATITEM_ID = trl.AD_PRINTFORMATITEM_ID "
						+ "AND pfi.IsCentrallyMaintained='Y') "
						//+ "AND trl.AD_Client_ID = 1000000"
						;
				no = DB.executeUpdate(sql, false, get_TrxName());	  	
				log.info("  rows updated: "+no);
				trx.commit(true);
